...
The results look promising: given any test slice image, the trained network can detect the locations of the four joints quite well. 
...
While the prediction of the joints appears qualitatively correct, more quantitative analysis would need to be done. Another issue is that the two joints can not be used to predict the profile of the furrow (one can simply draw a straight line which has no curvature information of the underlying furrow tissue). Can annotation of an additional joint in the middle of the furrow provide a better prediction profile?
